...
# ------------------------------------------------------------
import argparse, os, time, pathlib, traceback
...
for model_name, ctor in MODEL_CATALOG:
    ...
    try:
        ...
        outdir = RESULTS_DIR/model_name; outdir.mkdir(parents=True, exist_ok=True)

        # ─── inner optimiser loop ───
        for opt_name,cfg in opt_cfgs.items():
            ...
            pd.DataFrame(hist[opt_name]).to_csv(outdir/f"{opt_name}_metrics.csv", index=False)
